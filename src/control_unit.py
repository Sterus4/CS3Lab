...
class ControlUnit:
    ...
    def instruction_fetch(self):
        ...
        self.datapath.command_register.latch(
            self.datapath.instruction_memory.get_instruction()
        )
        self.tick()
    ...
    def address_fetch(self, instruction: Instruction):
        ...
        match instruction.addressing:
            ...
            case Addressing.IND_MEM:
                ...
                self.datapath.data_register.latch(
                    self.datapath.data_memory.get_data_out()
                )
                self.tick()
    ...
    def show_output(self, i: int):
        print(
            "Исполнено инструкций: {ins}, Тиков выполнено: {tick}".format(
                ins=i, tick=self.current_tick_counter
            )
        )
    ...
    def simulate(self):
        ...
        while i < self.instruction_limit:
            ...
            current_instruction: Instruction = (
                self.datapath.command_register.get_value()
            )

            if is_instruction_address(current_instruction):
                self.address_fetch(current_instruction)
            if (
                is_instruction_address(current_instruction)
                and current_instruction.opcode != Opcode.ST
            ):
                if current_instruction.addressing == Addressing.DIR:
                    self.datapath.data_register.latch(
                        self.datapath.command_register.get_value().operand
                    )
                    self.tick()
                else:
                    self.operand_fetch()
            ...
            match current_instruction.opcode:
                case Opcode.ADD:
                    ...
                    self.datapath.alu.latch_right(
                        self.datapath.data_register.get_value()
                    )
                    self.datapath.alu.add()
                ...
                case Opcode.PUSH:
                    self.datapath.data_address_register.latch(
                        self.datapath.sp_register.get_value() - 1
                    )
                    self.datapath.sp_register.latch(
                        self.datapath.sp_register.get_value() - 1
                    )
                    self.datapath.data_memory.latch_in()
                    self.tick()
                ...
                case Opcode.POP:
                    self.datapath.data_address_register.latch(
                        self.datapath.sp_register.get_value()
                    )
                    self.datapath.sp_register.latch(
                        self.datapath.sp_register.get_value() + 1
                    )
                ...
                case Opcode.JZ:
                    if self.datapath.flags[Flag.ZF]:
                        self.jump_to(current_instruction.operand)
                case Opcode.JNZ:
                    if not self.datapath.flags[Flag.ZF]:
                        self.jump_to(current_instruction.operand)
                case Opcode.JBZ:
                    if self.datapath.flags[Flag.ZF] or self.datapath.flags[Flag.NF]:
                        self.jump_to(current_instruction.operand)
                case Opcode.JB:
                    if (
                        not self.datapath.flags[Flag.ZF]
                        and self.datapath.flags[Flag.NF]
                    ):
                        self.jump_to(current_instruction.operand)
                case Opcode.JAZ:
                    if self.datapath.flags[Flag.ZF] or not self.datapath.flags[Flag.NF]:
                        self.jump_to(current_instruction.operand)
                case Opcode.JA:
                    if (
                        not self.datapath.flags[Flag.ZF]
                        and not self.datapath.flags[Flag.NF]
                    ):
                        self.jump_to(current_instruction.operand)
                case Opcode.HLT:
                    self.show_output(i)
