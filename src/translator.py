...
class Translator():
    ...
    def create_math(self, statement):
        ...
        if len(expression) != 1: self.create_operation(Opcode.POP)
    ...
    def create_comparison(self, left: str, right: str):
        ...
        self.create_operation(Opcode.SUB, self.current_free_data_address, Addressing.MEM)
    ...
    def add_print_pointer(self):  # В Acc адрес начала строки
        ...
        self.create_operation(Opcode.LD, self.current_free_data_address, Addressing.IND_MEM)
        ...
        while i < len(code):
            ...
            match self.recognise_token(current_token):
                case TokenType.CREATE_NEW_VAR:
                    ...
                    DataType.CHAR if var_type == "char" else DataType.INT, self.current_free_data_address)
                    if var_type == "int":
                        if operand is None:
                            self.create_operation(Opcode.LD, 0, Addressing.DIR)
                        else:
                        ...
                        if operand is None:
                            ...
                            raise VarException(
                                "Инициализировать char можно только числом или символом: " + current_token)
                ...
                case TokenType.CREATE_NEW_POINTER:
                    ...
                    if '=' in current_token:
                        ...
                        if operand == '': raise VarException(current_token + ": ожидалась инициализация")
                    self.variables[var_name] = (DataType.POINTER, self.current_free_data_address)
                    if var_capacity == -1 and operand is None: raise InvalidToken("Неизвестный токен: " + current_token)
                    if var_capacity == -1:
                        if self.recognise_token(operand) != TokenType.STRING: raise InvalidToken(
                            "Неизвестный токен: " + current_token)
                        self.create_operation(Opcode.LD, self.current_free_data_address + 1, Addressing.DIR)
                        self.create_operation(Opcode.ST, self.current_free_data_address, Addressing.MEM)
                        self.current_free_data_address += 1
                        operand = operand[1:-1]
                        for char in operand:
                        ...
                        if operand is not None and self.recognise_token(operand) != TokenType.STRING: raise InvalidToken(
                            "Неизвестный токен: " + current_token)
                        self.create_operation(Opcode.LD, self.current_free_data_address + 1, Addressing.DIR)
                        self.create_operation(Opcode.ST, self.current_free_data_address, Addressing.MEM)
                        self.current_free_data_address += 1
                        if operand is not None:
                            ...
                            for char in operand:
                                ...
                                count += 1
                                self.create_operation(Opcode.LD, ord(char), Addressing.DIR)
                                self.create_operation(Opcode.ST, local_free_data_address, Addressing.MEM)
                ...
                case TokenType.UPDATE_VAR:
                    ...
                    if var_name not in self.variables: raise VarException(
                        "Переменной " + var_name + " не существует: " + current_token)
                    if self.variables[var_name][0] == DataType.POINTER: raise VarException(
                    ...
                    if self.variables[var_name][0] == DataType.CHAR:
                        if is_char(operand):
                            self.create_operation(Opcode.LD, ord(operand[1]), Addressing.DIR)
                        ...
                        self.create_math(operand)
                    self.create_operation(Opcode.ST, self.variables[var_name][1], Addressing.MEM)
                case TokenType.IF:
                    if (self.recognise_token(code[i + 1]) != TokenType.QUOTE_ROUND_OPEN
                            or self.recognise_token(code[i + 2]) != TokenType.COMPARISON
                            or self.recognise_token(code[i + 3]) != TokenType.QUOTE_ROUND_CLOSE
                            or self.recognise_token(code[i + 4]) != TokenType.QUOTE_FIGURE_OPEN):
                        raise TranslateException("Неправильный формат if выражения: " + str(code[i:i + 4]))
                    start_of_block, end_of_block = i + 4, self.find_end_of_block(code, i + 4)
                    ...
                    self.result[comparison_index - self.result[0].address].operand = self.current_instruction_address
                    i = end_of_block
                case TokenType.WHILE:
                    ...
                    self.create_reverse_sign(comparison_sign)
                    self.create_code(code[start_of_block + 1: end_of_block])
                    self.result[comparison_index - self.result[0].address].operand = self.current_instruction_address + 1
                    self.create_operation(Opcode.JMP, while_start)
                    i = end_of_block
                case TokenType.PRINT:
                    if self.recognise_token(code[i + 1]) != TokenType.QUOTE_ROUND_OPEN or self.recognise_token(
                    ...
                    match self.recognise_token(code[i + 2]):
                        ...
                        case TokenType.VAR_VALUE:
                            if code[i + 2] not in self.variables: raise VarException(
                                "Переменной не существует: " + code[i + 2])
                            match self.variables[code[i + 2]][0]:
                                case DataType.CHAR:
                                    self.create_operation(Opcode.LD, self.variables[code[i + 2]][1], Addressing.MEM)
                                    self.create_operation(Opcode.ST, IO_OUT_MEM, Addressing.MEM)
                                case DataType.INT:
                                    self.create_operation(Opcode.LD, self.variables[code[i + 2]][1], Addressing.MEM)
                ...
                case TokenType.READ:
                    if self.recognise_token(code[i + 1]) != TokenType.QUOTE_ROUND_OPEN or self.recognise_token(
                            code[i + 3]) != TokenType.QUOTE_ROUND_CLOSE or self.recognise_token(
                        code[i + 2]) != TokenType.VAR_VALUE:
                        raise TranslateException("Неправильный формат: " + current_token)
                    var_name = code[i + 2]
                    if var_name not in self.variables: raise VarException("Переменной не существует: " + code[i + 2])
    ...
    def create_operation(self, opcode, operand=None, addressing=None):
        self.result.append(Instruction(self.current_instruction_address, opcode, operand, addressing))
